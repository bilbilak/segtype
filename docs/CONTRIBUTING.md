...
Before participating in the project, please read our [Code of Conduct](https://github.com/bilbilak/segtype?tab=coc-ov-file#readme). By engaging with this repository and its respective community, you agree to abide by its terms.

## 💻 Setting up your development environment

1. [Fork](https://github.com/bilbilak/segtype/fork) the project on _GitHub_ to your own account.
...
2. Clone your fork to your local development environment:
    ```
    git clone git@github.com:bilbilak/segtype.git
    cd segtype
    ```
3. Make sure you have the necessary tools installed, such as _TypeScript_ and any dependencies required by the project.
...
1. **Coding Conventions:** TypeScript in strict mode. Use single quotes, enforce semicolons, and maintain two-space indentation. Prefer `const` over `let`; avoid `var`. Use explicit return types on function signatures. Commit messages must follow [Conventional Commits](https://www.conventionalcommits.org/) with a [gitmoji](https://gitmoji.dev/) prefix.
...
3. **Cross-Platform Compatibility:** As a Web Component built on web standards, ensure your changes work across all modern browsers (Chrome, Firefox, Safari). Avoid browser-specific APIs unless polyfilled.
4. **Performance Optimization:** Keep the component lightweight — avoid adding runtime dependencies. SVG rendering should be efficient; minimize unnecessary re-renders and DOM operations.
5. **Testing:** Build and test with `npm run build` before submitting. Open `playground.html` in a browser to verify the playground renders correctly. Ensure no regressions in the existing glyph set.
